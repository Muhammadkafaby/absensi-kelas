...
function generateNavigation(role) {
    ...
    const navItems = [
        ...
        { text: 'Rekap Saya', action: 'recap-teacher', active: true },
...
async function loadRecapData() {
    ...
    try {
        ...
        const mockData = generateMockRecapData(dateStart, dateEnd, classFilter, subjectFilter);
...
function generateMockRecapData(dateStart, dateEnd, classFilter, subjectFilter) {
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
        ...
        for (let session = 1; session <= sessionsPerDay; session++) {
            const className = classFilter || ['X-1', 'X-2', 'XI-1', 'XI-2'][Math.floor(Math.random() * 4)];
    ...
    metrics.avgAttendance = sessionCount > 0 ? Math.round(totalAttendancePercent / sessionCount) : 0;
...
function updateMetrics(metrics) {
    ...
    document.getElementById('avgAttendance').textContent = `${metrics.avgAttendance}%`;
...
function downloadCsv() {
    ...
    const csvContent = [
        ...
        ...sessions.map(session => [
            session.tanggal, session.jam, session.kelas, session.mapel,
            session.h, session.a, session.s, session.i, session.percent
        ])
