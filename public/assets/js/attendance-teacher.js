...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Quick actions
    document.getElementById('allPresentBtn').addEventListener('click', setAllPresent);
    document.getElementById('resetBtn').addEventListener('click', resetAttendance);
...
function generateNavigation(role) {
    ...
    const navItems = [
        ...
        { text: 'Absensi', action: 'attendance-teacher', active: true },
        ...
        <tr data-nis="${student.nis}">
            ...
            <td>
                <div class="attendance-status">
                    <div class="status-option ${student.status === 'H' ? 'selected' : ''}" data-status="H">H</div>
                    <div class="status-option ${student.status === 'A' ? 'selected' : ''}" data-status="A">A</div>
                    <div class="status-option ${student.status === 'S' ? 'selected' : ''}" data-status="S">S</div>
                    <div class="status-option ${student.status === 'I' ? 'selected' : ''}" data-status="I">I</div>
                </div>
            </td>
            <td><input type="text" placeholder="Catatan" value="${student.catatan}" class="note-input"></td>
        </tr>
    `).join('');
...
function setAllPresent() {
    const statusOptions = document.querySelectorAll('#attendanceTableBody .status-option[data-status="H"]');
    const allStatusOptions = document.querySelectorAll('#attendanceTableBody .status-option');
...
async function saveAttendance() {
    ...
    // Validation
    if (!classSelect.value || !dateInput.value || !hourSelect.value || !subjectInput.value) {
    ...
    try {
        ...
        auth.showNotification(`Berhasil disimpan (${attendanceData.siswa.length} entri)`, 'success');
