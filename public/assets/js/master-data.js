...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('importStudentsBtn').addEventListener('click', () => importCsv('students'));
    document.getElementById('exportStudentsBtn').addEventListener('click', () => exportCsv('students'));
    document.getElementById('importTeachersBtn').addEventListener('click', () => importCsv('teachers'));
    document.getElementById('exportTeachersBtn').addEventListener('click', () => exportCsv('teachers'));
...
function generateNavigation(role) {
    ...
    const navItems = [
        ...
        { text: 'Master Data', action: 'master-data', active: true },
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            const subjects = [
                ...
                'Ekonomi', 'Sosiologi', 'Bahasa Jawa', 'PKN'
            ];
            resolve(subjects);
        }, 200);
    });
...
function generateMockTeachers() {
    const teachers = [
        ...
        { nama: 'Bu Siti', mapel: 'Bahasa Indonesia', username: 'siti', role: 'guru' },
        { nama: 'Pak Budi', mapel: 'Bahasa Inggris', username: 'budi', role: 'guru' },
        ...
        { nama: 'Pak Baru', mapel: 'Bahasa Inggris', username: 'baru', role: 'guru' },
...
function renderClassesTable(classes) {
    ...
    const rowsHtml = classes.map(className => `
        <tr>
            <td>${className}</td>
        </tr>
    `).join('');
...
function processCsvImport(type, csv) {
    ...
    // Basic validation
    if (type === 'students' && (!headers.includes('nis') || !headers.includes('nama') || !headers.includes('kelas'))) {
        auth.showNotification('Format CSV tidak valid untuk siswa', 'error');
        return;
    }

    if (type === 'teachers' && (!headers.includes('nama') || !headers.includes('mapel') || !headers.includes('username'))) {
    ...
    auth.showNotification(`Berhasil mengimpor ${data.length} data ${type}`, 'success');
