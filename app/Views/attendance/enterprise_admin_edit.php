...
<form action="<?= base_url('/attendance/admin/update/' . $session['id']) ?>" method="POST" id="attendanceForm">
    <?= csrf_field() ?>
    ...
    <div class="enterprise-card mb-6">
        <div class="card-header">
            <div>
                <h2 class="card-title">✏️ Edit Absensi (Admin)</h2>
                <p class="card-subtitle">Ubah data absensi</p>
            </div>
        ...
        <div class="form-row">
            <div class="enterprise-form-group">
                ...
                <select id="classSelect" name="class_id" class="enterprise-select" required>
                    ...
                    <?php foreach ($classes as $class): ?>
                        <option value="<?= $class['id'] ?>" <?= $session['class_id'] == $class['id'] ? 'selected' : '' ?>><?= esc($class['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="enterprise-form-group">
                ...
                <input type="date" id="dateInput" name="date" value="<?= $session['date'] ?>" class="enterprise-input" required>
            </div>
            ...
            <div class="enterprise-form-group">
                ...
                <select id="hourSelect" name="lesson_hour" class="enterprise-select">
                    ...
                    <?php for ($i = 1; $i <= 10; $i++): ?>
                        <option value="<?= $i ?>" <?= $session['lesson_hour'] == $i ? 'selected' : '' ?>>Jam ke-<?= $i ?></option>
                    <?php endfor; ?>
                </select>
            </div>
            ...
            <div class="enterprise-form-group">
                ...
                <select id="subjectSelect" name="subject_id" class="enterprise-select" required>
                    ...
                    <?php foreach ($subjects as $subject): ?>
                        <option value="<?= $subject['id'] ?>" <?= $session['subject_id'] == $subject['id'] ? 'selected' : '' ?>><?= esc($subject['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="enterprise-form-group">
                ...
                <select id="teacherSelect" name="teacher_id" class="enterprise-select" required>
                    ...
                    <?php foreach ($teachers as $teacher): ?>
                        <option value="<?= $teacher['id'] ?>" <?= $session['teacher_id'] == $teacher['id'] ? 'selected' : '' ?>><?= esc($teacher['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
        </div>
...
let students = [];
let attendanceData = {};
let existingRecords = <?= json_encode($records ?? []) ?>;

// Pre-load students for existing session
...
document.getElementById('classSelect').addEventListener('change', loadExistingData);
        ...
        <tr>
            ...
            <td>
                <select name="attendance[${student.id}]" class="enterprise-select" onchange="updateCounts()" required style="font-size: 0.875rem; padding: 0.5rem;">
                    ...
                    <option value="T" ${status === 'T' ? 'selected' : ''}>⏰ Terlambat</option>
                </select>
            </td>
            <td>
                <input type="text" name="note[${student.id}]" class="enterprise-input" placeholder="Catatan (opsional)" style="font-size: 0.875rem; padding: 0.5rem;" value="${note}">
            </td>
        </tr>
        `;
    }).join('');
