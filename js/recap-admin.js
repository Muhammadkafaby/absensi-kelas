...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Date filter change
    document.getElementById('absentDateFilter').addEventListener('change', loadDailyRecap);
...
function generateNavigation(role) {
    ...
    const navItems = [
        ...
        { text: 'Rekap Harian', action: 'recap-admin', active: true },
...
function renderClassRecapTable(classRecap) {
    ...
    const rowsHtml = classRecap.map(recap => `
        <tr>
            <td>${recap.kelas}</td>
...
function downloadCsv() {
    ...
    csvContent += ['No', 'NIS', 'Nama', 'Kelas', 'Mapel', 'Jam', 'Guru', 'Catatan'].join(',') + '\n';
    csvContent += absentData.map(row => [
        row.no, row.nis, row.nama, row.kelas, row.mapel, row.jam, row.guru, row.catatan
    ].join(',')).join('\n');

    csvContent += '\n\nRekap per Kelas\n';
    csvContent += ['Kelas', 'H', 'A', 'S', 'I', '% Hadir'].join(',') + '\n';
    csvContent += classRecapData.map(row => [
        row.kelas, row.h, row.a, row.s, row.i, row.percent
    ].join(',')).join('\n');
